...
class _DetalleProdcutoPageState extends State<DetalleProdcutoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(producto.selectedProduct.name, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.bold),),
        centerTitle: true,
        elevation: 0,
      ),
      body: Column(
        children: [
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                const Text("Descripción del producto:", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black),),
                const SizedBox(height: 10,),
                Text(producto.selectedProduct.descripcion, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.normal, color: Colors.black) ),
                ...
                Row(
                  ...
                  children: [
                    MaterialButton(
                      ...
                      elevation: 3,
                      color: Colors.red,
                      child: const Icon(Icons.remove_rounded, size: 40,),
                      onPressed: (){
                        if(cantidad > 0) {
                          cantidad = cantidad - 1;
                          setState(() {});
                ...
                MaterialButton(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: const [
                        Text('Agregar', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white),),
                        SizedBox(width: 10,),
                        Icon(Icons.shopping_cart_checkout_sharp, size: 35, color: Colors.white,),
                      ],
                    ),
                  ),
                  onPressed: cantidad <= 0 ? null : () async {
                    
                    final carritoService = Provider.of<CarritoService>(context, listen: false);

                    carritoService.isLoading = true;
                    final String? message = await carritoService.createCarrito(
                      Carrito(
